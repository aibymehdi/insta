 ...
 یه کتابخونه پایتونی برای *وب‌اسکرپینگ*ه که از LLM و منطق مستقیم گرافی استفاده می‌کنه تا برای سایت‌ها و فایل‌های محلی مثل XML، HTML، JSON، Markdown و چیزهای مشابه، پایپ‌لاین اسکرپینگ بسازه.

کافیه بگی چه اطلاعاتی رو می‌خوای استخراج کنی؛ بقیه‌اش رو خود کتابخونه برات انجام می‌ده.

## 🚀 یکپارچه‌سازی‌ها

ابزار ScrapeGraphAI خیلی راحت با فریم‌ورک‌ها و ابزارهای معروف وصل می‌شه تا توانایی اسکرپینگت رو بیشتر کنه. فرقی نداره با Python کار کنی یا Node.js، از فریم‌ورک‌های LLM استفاده کنی یا سراغ پلتفرم‌های no-code بری؛ اینجا برای همه‌شون گزینه‌های یکپارچه‌سازی آماده شده.
